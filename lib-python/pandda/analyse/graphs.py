...
def write_individual_dataset_plots(pandda, datasets):
    ...
    for d in datasets:
        ...
        # Plot settings
        pyplot.axes().set_xticklabels([round(float(t)**-0.5,2) if (t>0.0) else '' for t in pyplot.axes().get_xticks()])
...
def write_dataset_summary_graphs(pandda):
    ...
    axis_2.set_xticklabels([round(float(t)**-0.5,2) if (t>0.0) else '' for t in axis_2.get_xticks()])
