...
import cctbx.maptbx
import scitbx.sparse, scitbx.matrix
...
from giant.structure.select import calphas, protein
...
from pandda.analyse.functions import DatasetAligner, MapLoader, DensityStatistics, wrapper_run
...
class PanddaMultiDatasetAnalyser(Program):
    ...
    def _directory_setup(self):
        ...
        fm.add_dir(dir_name='dataset_graph_summaries', dir_tag='d_graphs', top_dir_tag='analyses', create=False, exists=False)
        ...
        fm.add_dir(dir_name='results_summaries', dir_tag='output_summaries', top_dir_tag='root', create=False, exists=False)
        ...
        fm.add_dir(dir_name='pickled_panddas', dir_tag='pickles', top_dir_tag='root', create=False, exists=False)
        ...
        # ===============================================================================>
        fm.add_dir(dir_name='statistical_maps', dir_tag='statistical_maps', top_dir_tag='root', create=False, exists=False)
    ...
    def load_reference_dataset(self, ref_pdb, ref_mtz):
        ...
        sites_min = calphas(ref_dataset.model.hierarchy).atoms().extract_xyz().min()
        ...
        if not os.path.exists(self.file_manager.get_file('reference_on_origin')):
            tmp_r_hierarchy.atoms().set_xyz(ref_dataset.model.alignment.nat2ref(tmp_r_hierarchy.atoms().extract_xyz()))
        ...
        if not os.path.exists(self.file_manager.get_file('reference_symmetry')):
            ...
            ref_sym_copies.atoms().set_xyz(ref_dataset.model.alignment.nat2ref(ref_sym_copies.atoms().extract_xyz()))
    ...
    def create_reference_grid(self, dataset, grid_spacing):
        ...
        sites_cart = dataset.model.hierarchy.atoms().extract_xyz()
        buffer = self.params.masks.outer_mask + self.params.maps.padding

        grid_min = flex.double([s-buffer for s in sites_cart.min()])
        grid_max = flex.double([s+buffer for s in sites_cart.max()])

        self.grid = Grid(grid_spacing=grid_spacing, origin=(0,0,0), approx_max=tuple(grid_max-grid_min), verbose=self.settings.verbose)
        ...
        if self.params.alignment.method == 'local':
            ...
            site_cart_ca = dataset.model.alignment.nat2ref(calphas(dataset.model.hierarchy).atoms().extract_xyz())

            t1 = time.time()
            self.grid.create_grid_partition(sites_cart=site_cart_ca)
            self.grid.partition.partition(cpus=self.settings.cpus)
            t2 = time.time()
    ...
    def mask_reference_grid(self, dataset):
        ...
        mask_map_file = self.file_manager.get_file('reference_dataset').replace('.mtz','.totalmask.ccp4')
        map_mask = flex.double(self.grid.global_mask().total_mask_binary().astype(int))
        map_mask.reshape(flex.grid(self.grid.grid_size()))
        write_array_to_map(output_file=mask_map_file, map_data=map_mask, grid=self.grid)
        # Write symmetry masked map
        mask_map_file = self.file_manager.get_file('reference_dataset').replace('.mtz','.symmask.ccp4')
        map_mask = flex.double(self.grid.symmetry_mask().total_mask_binary().astype(int))
        map_mask.reshape(flex.grid(self.grid.grid_size()))
        write_array_to_map(output_file=mask_map_file, map_data=map_mask, grid=self.grid)
...
#            dataset.file_manager.add_file(file_name=f.ligand_image.format(dataset.tag),                       file_tag='ligand_image'         )
    ...
    def load_diffraction_data(self):
        ...
        for dataset in self.datasets.mask(mask_name='rejected - total', invert=True):
            if all_cols in dataset.data.structure_factors.keys():
                print('\rAlready Loaded: Dataset {!s}                           '.format(dataset.tag), end=''); sys.stdout.flush()
            else:
    ...
    def align_datasets(self, method):
        ...
        for alignment in dataset_alignments:
            print('Aligning Structures: Dataset {!s}'.format(alignment.id), end=''); sys.stdout.flush()
            ...
            alignment_rmsd = dataset.model.alignment.alignment_rmsd()
            print('\rStructure Aligned: Dataset {!s} - rmsd {}'.format(alignment.id, round(alignment_rmsd,3)))

        t2 = time.time()
        self.log('> Aligning Structures > Time Taken: {!s} seconds'.format(int(t2-t1)), True)
    ...
    def calculate_dataset_rmsds_to_reference(self):
        ...
        for d in self.datasets.mask(mask_name='rejected - total', invert=True):
            rmsd = d.model.alignment.alignment_rmsd()
...
#                t_shift =  rt_1_2.t.norm_sq()**0.5
    ...
    def filter_datasets_1(self, filter_dataset=None):
        ...
        for dataset in self.datasets.mask(mask_name='rejected - total', invert=True):
            ...
            if dataset.model.space_group.info().symbol_and_number() != filter_dataset.model.space_group.info().symbol_and_number():
                ...
                self.datasets.all_masks().set_mask_value(mask_name='bad structure - different space group', entry_id=dataset.tag, value=True)
            # Check that the hierarchies are identical
            if not dataset.model.hierarchy.is_similar_hierarchy(filter_dataset.model.hierarchy):
                self.log('\rRejecting Dataset: {!s}          '.format(dataset.tag))
                ...
                self.datasets.all_masks().set_mask_value(mask_name='bad structure - non-identical structures', entry_id=dataset.tag, value=True)
    ...
    def filter_datasets_2(self):
        ...
        for dataset in self.datasets.mask(mask_name='rejected - total', invert=True):
            ...
            if dataset.model.input.get_r_rfree_sigma().r_free > self.params.filtering.max_rfree:
                self.log('\rRejecting Dataset: {!s}          '.format(dataset.tag))
                ...
                self.datasets.all_masks().set_mask_value(mask_name='bad crystal - rfree', entry_id=dataset.tag, value=True)
            # Check the deviation from the average sites
            elif dataset.model.alignment.alignment_rmsd() > self.params.filtering.max_rmsd_to_reference:
                self.log('\rRejecting Dataset: {!s}          '.format(dataset.tag))
                ...
                self.datasets.all_masks().set_mask_value(mask_name='bad crystal - isomorphous structure', entry_id=dataset.tag, value=True)
            else:
                pass
    ...
    def reset_loaded_datasets(self):
        ...
        for dataset in self.datasets.all():
            if self.args.method.reprocess_existing_datasets or (dataset.tag in datasets_for_reprocessing):
    ...
    def select_for_building_distributions(self, high_res_cutoff, building_mask_name):
        ...
        for dataset in self.datasets.mask(mask_name='rejected - total', invert=True):
            ...
            elif self.datasets.all_masks().get_mask_value(mask_name='no_build', entry_id=dataset.tag) == True:
                self.log('Rejecting Dataset {!s}: Excluded from building'.format(dataset.tag))
                continue
            else:
                self.datasets.all_masks().set_mask_value(mask_name=building_mask_name, entry_id=dataset.tag, value=True)
    ...
    def select_for_analysis(self, high_res_large_cutoff, high_res_small_cutoff, analysis_mask_name):
        ...
        for dataset in self.datasets.mask(mask_name='rejected - total', invert=True):
            ...
            # Check the resolution of the dataset (is not too high)
                ...
                continue
            # Check to see if this has been excluded from building
            elif self.datasets.all_masks().get_mask_value(mask_name='no_analyse', entry_id=dataset.tag) == True:
                self.log('Rejecting Dataset {!s}: Excluded from analysis'.format(dataset.tag))
                continue
            elif self.datasets.all_masks().get_mask_value(mask_name='old datasets', entry_id=dataset.tag) and (not self.args.method.reprocess_existing_datasets) and (dataset.tag not in datasets_for_reprocessing):
                self.log('Rejecting Dataset {!s}: Already Processed (Old Dataset)'.format(dataset.tag))
                continue
            else:
                self.datasets.all_masks().set_mask_value(mask_name=analysis_mask_name, entry_id=dataset.tag, value=True)
        return self.datasets.all_masks().get_mask(analysis_mask_name)

    def truncate_scaled_data(self, datasets):
    ...
    def load_reference_map(self, map_resolution=0):
        ...
        # Extract the points for the map
    ...
    def load_and_morph_maps(self, datasets, ref_map, map_resolution=0):
        ...
        finish = time.time()
        self.log('\r> FFT-ing Maps ({!s} Datasets) > Time Taken: {!s} seconds'.format(len(datasets), int(finish-start)), True)
        ...
        finish = time.time()
        self.log('\r> Loading Maps ({!s} Datasets) > Time Taken: {!s} seconds'.format(map_list.size(), int(finish-start)), True)

        # Set the mask lengths and entry ids
        map_list.all_masks().set_mask_length(mask_length=map_list.size())
        map_list.all_masks().set_entry_ids(entry_ids=[m.meta.tag for m in map_list.all()])
    ...
    def cluster_events_and_update(self, events=[], update_tables=True, update_output=True):
        ...
        [s.find_protein_context(hierarchy=self.datasets.reference().hierarchy) for s in site_list.children]
...
#                print(c.err)
    ...
    def write_map_analyser_maps(self, map_analyser, analysis_mask_name):
        ...
        write_array_to_map(output_file=self.file_manager.get_file('mean_map').format(map_res),
                           map_data=map_analyser.statistical_maps.mean_map.data, grid=self.grid)
        write_array_to_map(output_file=self.file_manager.get_file('stds_map').format(map_res),
                           map_data=map_analyser.statistical_maps.stds_map.data, grid=self.grid)
        write_array_to_map(output_file=self.file_manager.get_file('sadj_map').format(map_res),
                           map_data=map_analyser.statistical_maps.sadj_map.data, grid=self.grid)
        write_array_to_map(output_file=self.file_manager.get_file('skew_map').format(map_res),
                           map_data=map_analyser.statistical_maps.skew_map.data, grid=self.grid)
        write_array_to_map(output_file=self.file_manager.get_file('kurt_map').format(map_res),
                           map_data=map_analyser.statistical_maps.kurt_map.data, grid=self.grid)
        write_array_to_map(output_file=self.file_manager.get_file('bimo_map').format(map_res),
                           map_data=map_analyser.statistical_maps.bimo_map.data, grid=self.grid)
    ...
    def update_site_table(self, site_list, clear_table=True):
        ...
        for site in site_list.children:
            ...
            self.tables.site_info.set_value(site.id, 'native_centroid', tuple(self.dataset.reference().ref2nat(coordinates=[centroid_cart])[0]))
    ...
    def add_event_to_event_table(self, dataset, event):
        ...
        self.tables.event_info.set_value(event.id, ['x','y','z'], list(dataset.model.alignment.ref2nat(points=self.grid.grid2cart([event.cluster.peak],origin=False))[0]))
...
class PanddaMapAnalyser(object):
    ...
    def __init__(self, dataset_maps, meta, statistical_maps=None, parent=None, log=None):
        ...
        assert hasattr(meta, 'map_data_size')
        self.meta = meta
    ...
    def validate_maps(self):
        ...
        for m in self.dataset_maps.all():
            self.log('Checking Map {!s}'.format(m.tag))
            assert m.data.size() == self.meta.map_data_size, 'Not all maps are the same size'
    ...
    def calculate_mean_map(self, mask_name=None):
        ...
        self.statistical_maps.mean_map = m.new_from_template(map_data=flex.double(mean_map_vals.tolist()), sparse=True)
        self.statistical_maps.medn_map = m.new_from_template(map_data=flex.double(medn_map_vals.tolist()), sparse=True)
    ...
    def calculate_map_uncertainties(self, masked_idxs=None, mask_name=None, q_cut=1.5, cpus=1):
        ...
        self.log('----------------------------------->>>')
        self.log('Calculating Map Uncertainties (using {!s} cores) for {} datasets'.format(cpus, len(arg_list)))
        t1 = time.time()

        arg_list = []
        for i_m, m in enumerate(self.dataset_maps.mask(mask_name=mask_name)):
            if m.meta.map_uncertainty is not None:
                print('SKIPPING Dataset {!s} ({!s}/{!s})'.format(m.tag, i_m+1, self.dataset_maps.size(mask_name=mask_name)))
                arg_list.append(None)
                continue
            u = UncertaintyCalculator(query_values=m.data.select(masked_idxs), ref_values=mean_vals, file_manager=m.parent.file_manager)
            arg_list.append(u)

        map_uncertainties = easy_mp.pool_map(func=wrapper_run, args=arg_list, processes=self.settings.cpus, chunksize=1)

        for i_m, m in enumerate(self.dataset_maps.mask(mask_name=mask_name)):
            map_unc = map_uncertainties[i_m]
            if m.meta.map_uncertainty:
    ...
    def calculate_statistical_maps(self, mask_name=None, ignore_warnings=True, cpus=1):
        ...
        self.log('----------------------------------->>>', True)
        self.log('Calculating statistics of {} grid points (using {!s} cores)'.format(self.meta.map_data_size, cpus), True)
        ...
        point_statistics = numpy.empty((self.meta.map_data_size, 5))
        ...
        for i_chunk, chunk_start in enumerate(chunk_idxs):
            ...
            for i_chunk_2, chunk_start_2 in enumerate(chunk_idxs_2):
                ...
                map_vals = numpy.array([m.data[chunk_start+chunk_start_2:chunk_start+chunk_start_2+chunk_size_2] for m in self.dataset_maps.mask(mask_name=mask_name)])

                if i_chunk_2+1 < num_chunks_2:
                    assert len(map_vals) == self.dataset_maps.size(mask_name=mask_name)
                    assert len(map_vals.T) == chunk_size_2
            ...
            for i_chunk_2, chunk_start_2 in enumerate(chunk_idxs_2):
                point_statistics[chunk_start+chunk_start_2:chunk_start+chunk_start_2+chunk_size_2,:] = tmp_point_statistics[i_chunk_2]
        ...
        self.statistical_maps.stds_map = self.statistical_maps.mean_map.new_from_template(map_data=flex.double(point_statistics[:,0].tolist()), sparse=True)
        self.statistical_maps.sadj_map = self.statistical_maps.mean_map.new_from_template(map_data=flex.double(point_statistics[:,1].tolist()), sparse=True)
        self.statistical_maps.skew_map = self.statistical_maps.mean_map.new_from_template(map_data=flex.double(point_statistics[:,2].tolist()), sparse=True)
        self.statistical_maps.kurt_map = self.statistical_maps.mean_map.new_from_template(map_data=flex.double(point_statistics[:,3].tolist()), sparse=True)
        self.statistical_maps.bimo_map = self.statistical_maps.mean_map.new_from_template(map_data=flex.double(point_statistics[:,4].tolist()), sparse=True)
    ...
    def calculate_z_map(self, method, map=None, tag=None, uncertainty=None):
        ...
        # Calculate Z-values
        if method == 'naive':
            z_map = (map - self.statistical_maps.mean_map.data)/self.statistical_maps.stds_map.data
        elif method == 'adjusted':
            z_map = (map - self.statistical_maps.mean_map.data)/self.statistical_maps.sadj_map.data
        elif method == 'uncertainty':
            z_map = (map - self.statistical_maps.mean_map.data)/uncertainty
        elif method == 'adjusted+uncertainty':
            z_map = (map - self.statistical_maps.mean_map.data)/flex.sqrt(self.statistical_maps.sadj_map.data**2 + uncertainty**2)
        else:
