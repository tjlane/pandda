...
from Bamboo.Utils.Constants import MapTypes

# Contains Functions for Manipulating MTZs
...
def select_factors_and_phases_for_map(cols, maptype='2FOFC'):
    ...
    if maptype not in MapTypes:
        raise Exception('MapType not in allowed map types: {!s} not in {!s}'.format(maptype,MapTypes))
