...
def calculate_pairwise_distances_between_mols(mol1, mol2):
    ...
    mol1_idxs = [a.GetIdx() for a in mol1.GetAtoms() if a.GetMonomerInfo().GetResidueName() not in waterNames]
    mol2_idxs = [a.GetIdx() for a in mol2.GetAtoms() if a.GetMonomerInfo().GetResidueName() not in waterNames]
